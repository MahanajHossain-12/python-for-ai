...
list1.remove("Sanjo")# remove removes the first occurrence of a specific element from the list.
list1.insert(2, "Inserted")# insert adds an element at a specific index, shifting the rest of the elements to the right.
list1.pop(3)# pop removes the last element by default, but you can specify an index to remove a specific element.
list1.sort(key=str)# sort sorts the list in ascending order. The key parameter allows you to specify a function to be called on each list element before making comparisons. In this case, str is used to convert all elements to strings for comparison.
print(list1)
...
list1.extend(["New1", "New2"])# extend adds all elements from an iterable (like another list) to the end of the list.
list1.count("New1")# count returns the number of occurrences of a specific element in the list.
list1.index("New1")# index returns the index of the first occurrence of a specific element in the list. If the element is not found, it raises a ValueError.
print(list1)
...
print(my_set)
my_set.discard(10) # Discarding an element from the set (does not raise an error if the element is not found)
...
Total_1 = calculate_total(100, 0.1, 5)  # Calling the function with price, tax_rate, and discount
Total_2 = calculate_total(200, 0.15, 10)  # Calling the function with different arguments
...
result_double = double_value(5)  # Calling the function with an argument, output will be 10
...
if double_value(3) > 5:  # Calling the function within an if statement
    print(f"The doubled value is greater than 5: {double_value(3)}")  # This will not print since double_value(3) is 6
...
print(f"Minimum value: {min_val}, Maximum value: {max_val}")  # Printing the minimum and maximum values
...
from math import sqrt, pi # Importing specific functions (sqrt) and constants (pi) from the math module
...
import pandas as pd
data = {'Name': ['Alice', 'Bob', 'Charlie'], 'Age': [25, 30, 35]}  # Creating a dictionary with sample data
...
# Make the request
response = requests.get(url) # Making a GET request to the specified URL to retrieve weather data
data = response.json() # Parsing the JSON response from the API into a Python dictionary

temperature = data['current']['temperature_2m'] # Extracting the current temperature from the parsed data
