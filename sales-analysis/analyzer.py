...
)  # Prints the current working directory to help verify that the script is being run from the correct location.
...
if os.path.exists(
    data_path
):  # Check if the sales data file exists at the specified path.
    print(f" Found {data_path}")
else:
    print(
        f" Cannot find. Make sure you're running from the sales-analysis folder! {data_path}"
    )
...
product_totals = df.groupby("product", as_index=False).agg(
    total_quantity=("quantity", "sum"), total_cost=("total", "sum")
)
